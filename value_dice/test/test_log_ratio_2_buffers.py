...
from value_dice.log_ratio_from_2_buffer import Log_Ratio_from_two_buffer
...
val = str("1_4")
exp = 1
#buff1 = torch.load("old_buffers/Pendulum/l_1_4/mem")
#buff2 = torch.load("old_buffers/Pendulum/l_1_5/mem")

buff1 = torch.load("old_buffers/Pendulum/FIFO/l_1_0/mem")
buff2 = torch.load("old_buffers/Pendulum/FIFO/l_1_4/mem")
...
nu_nn_param = NN_Paramters(state_dim, action_dim, hidden_layer_dim=[256,256],
                          non_linearity=torch.relu, device=torch.device("cuda"), l_r=0.00001)
...
KL = Log_Ratio_from_two_buffer( nu_param=nu_nn_param, algo_param=log_algo_param, deterministic_env=False, averege_next_nu=True,
...
for i in range(60000):
    ...
    KL.train_ratio(buff1.sample(batch_size=batch_size), buff2.sample(batch_size=batch_size), unweighted=True)
    ...
    if i%eval_interval==0:
        ...
        print("KL at itr " + str(i) + " = " + str(KL.get_KL(buff1.sample(batch_size=batch_size), buff2.sample(batch_size=batch_size), unweighted=True)) )
        print(KL.loss1, KL.loss2)
        #print(KL.u_loss1)
        #print(KL.x)
