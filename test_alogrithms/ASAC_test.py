...
from algorithms.ASAC import ASAC
...
A = ASAC(env, q_nn_param, policy_nn_param, algo_nn_param, max_episodes=200, memory_capacity=100000
        ,batch_size=256, alpha_lr=0.0003, no_online_steps_per_update=100)
...
for i in range(100000):
    batch = A.sample_mem()
    A.update(batch=batch)
    batch = A.sample_online()
    A.update(batch=batch)

    if i*A.no_online_steps_per_update < A.batch_size:
        state = A.step(state, random=True)
    else:
        state = A.step(state, random=False)
    if i*A.no_online_steps_per_update%save_interval==0:
        #A.save("q1", "q2", "q1_target", "q2_target", "policy_target")
        pass
    if i*A.no_online_steps_per_update%eval_interval==0:
