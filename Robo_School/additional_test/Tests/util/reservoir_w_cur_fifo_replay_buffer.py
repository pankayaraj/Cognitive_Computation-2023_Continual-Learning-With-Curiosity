...
class Half_Reservoir_w_Cur_FIFO_Replay_Buffer():
    ...
    def push(self, state, action, action_mean, reward, curiosity, next_state, done_mask):
        ...
        if ran < self.fifo_frac:
            ...
            self.reservior_buffer.push(state, action, action_mean, reward, curiosity, next_state, done_mask)

    def sample(self, batch_size):
        fifo_indices, reservior_indices = self.get_sample_indices(batch_size)
        state, action, action_mean, reward, curiosity, next_state, done_mask = self.encode_sample(fifo_indices, reservior_indices)
    ...
    def encode_sample(self, fifo_indices, reservior_indices):
        ...
        s2, a2, a_m2, r2, c2, n_s2, d2 = self.reservior_buffer.encode_sample(reservior_indices)
        c1 = [0 for i in range(len(fifo_indices))]
