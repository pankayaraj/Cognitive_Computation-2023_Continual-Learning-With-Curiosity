...
class Replay_Memory_MTR():
    ...
    def push(self, state, action, action_mean, reward, next_state, done_mask, t):
        data = (state, action, action_mean, reward, next_state, done_mask, t)
    ...
    def sample(self, batch_size):
        ...
        state, action, action_mean, reward, next_state, done_mask, t = self.encode_sample(indices)

        return Transition_tuple(state, action, action_mean, reward, next_state, done_mask, t)

    def encode_sample(self, indices):
        state, action, action_mean, reward, next_state, done_mask, t = [], [], [], [], [], [], []
        for i in indices:
            ...
            t.append(self.storage[i][6])
        return state, action, action_mean, reward, next_state, done_mask, t
