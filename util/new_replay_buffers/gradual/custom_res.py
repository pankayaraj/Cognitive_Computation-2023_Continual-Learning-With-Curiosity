...
class Custom_Res():
    ...
    def push(self, state, action, action_mean, reward,  next_state, done_mask, tiebreaker):
        self.check_for_task_change()
        data = (state, action, action_mean, reward, next_state, done_mask, tiebreaker)
    ...
    def sample(self, batch_size):
        ...
        state, action, action_mean, reward, next_state, done_mask, t_array = self.encode_sample(
            indices=indices)
        return Transition_tuple(state, action, action_mean, reward, next_state, done_mask, t_array)
    ...
    def encode_sample(self, indices):
        ...
        return state, action, action_mean, reward, next_state, done_mask, t_array
