...
class Transition_tuple():
    ...
    def __init__(self, state, action, action_mean, reward, next_state, done_mask, time_step):
    ...
    def get_all_attributes(self):
        return [self.state, self.action,  self.action_mean, self.reward, self.next_state, self.done_mask, self.time_step]
...
class Replay_Memory_TR():
    ...
    def sample(self, batch_size):
        ...
        state, action, action_mean, reward, next_state, done_mask, time_step = self.encode_sample(indices)

        return Transition_tuple(state, action, action_mean, reward, next_state, done_mask, time_step)
    ...
    def encode_sample(self, indices):
        ...
        for i in indices:
            ...
            time_step.append(self.storage[i][6])
