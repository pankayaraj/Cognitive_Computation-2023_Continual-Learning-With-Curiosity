...
class Custom_Res_TR_P():
    ...
    def push(self, state, action, action_mean, reward,  next_state, done_mask, tiebreaker, initial_state, time_step, mean, std):
        self.check_for_task_change()
        data = (state, action, action_mean, reward, next_state, done_mask, tiebreaker, initial_state, time_step, mean, std)
    ...
    def sample(self, batch_size):
        ...
        state, action, action_mean, reward, next_state, done_mask, t_array, initial_state, time_step, mean, std = self.encode_sample(
            indices=indices)
        return Transition_tuple(state, action, action_mean, reward, next_state, done_mask, t_array, initial_state, time_step, mean, std)


    def encode_sample(self, indices):
        state, action, action_mean, reward, next_state, done_mask, t_array, initial_state, time_step, mean, std = [], [], [], [], [], [], [], [], [], [], []
        for (j,idxs) in enumerate(indices):
            ...
            for i in idxs:
                ...
                s, a, a_m, r, n_s, d, t, i_s, t_s, m, st = data
                state.append(s)
                action.append(a)
        ...
        return state, action, action_mean, reward, next_state, done_mask, t_array, initial_state, time_step, mean, std
