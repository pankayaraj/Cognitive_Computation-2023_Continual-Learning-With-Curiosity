import numpy as np
from util.new_replay_buffers.task_relevance_policy.replay_buff_cur import Replay_Memory_Cur_TR_P
from util.new_replay_buffers.task_relevance_policy.gradual_task_with_relevance.reservoir_task_seperation_gradual import Reservoir_Task_Seperation_Replay_Memory_Gradual_TR_P
import random
from itertools import count

class Transition_tuple():

    def __init__(self, state, action, action_mean, reward, curiosity, next_state, done_mask, t, initial_state, time_step, mean, std):
    ...
    def get_all_attributes(self):
        return [self.state, self.action,  self.action_mean, self.reward, self.curiosity, self.next_state, self.done_mask, self.t, self.initial_state, self.time_step, self.mean, self.std]
...
class Half_Reservoir_Flow_Through_w_Cur_Gradual_TR():
    ...
    def __init__(self, capacity=10000, curisoity_buff_frac = 0.34, seperate_cur_buffer=True,  priority = "uniform",
        ...
        self.curiosity_buffer = Replay_Memory_Cur_TR_P(capacity=int(capacity*curisoity_buff_frac))
        self.seperate_cur_buffer = seperate_cur_buffer

        self.fifo_buffer = Replay_Memory_Cur_TR_P(capacity=self.fifo_capacity)
        self.reservior_buffer = Reservoir_Task_Seperation_Replay_Memory_Gradual_TR_P(capacity=self.reservior_capacity,
    ...
    def push(self, state, action, action_mean, reward, curiosity, next_state, done_mask, initial_state, time_step, mean, std):
        self.t += 1

        old_data = self.fifo_buffer.push(state, action, action_mean, reward, curiosity, next_state, done_mask, initial_state, time_step, mean, std)
        if old_data != None:
            state, action, action_mean, reward, curiosity, next_state, done_mask, initial_state, time_step = old_data
            self.reservior_buffer.push(state, action, action_mean, reward, curiosity, next_state, done_mask, None, initial_state, time_step, mean, std)

        self.curiosity_buffer.push(state, action, action_mean, reward, curiosity, next_state, done_mask, initial_state, time_step, mean, std)

    def sample(self, batch_size):

        fifo_indices, reservoir_indices = self.get_sample_indices(batch_size)
        state, action, action_mean, reward, curiosity, next_state, done_mask, initial_state, time_step, mean, std = self.encode_sample(fifo_indices, reservoir_indices)
        ...
        return Transition_tuple(state, action, action_mean, reward, curiosity, next_state, done_mask, None, initial_state, time_step, mean, std)
    ...
    def encode_sample(self, fifo_indices, reservior_indices):
        ...
        state, action, action_mean, reward, curiosity, next_state, done_mask, initial_state, time_step, mean, std = [], [], [], [], [], [], [], [], [], [], []
        s1, a1, a_m1, r1, c1, n_s1, d1, i_s1, t_s1, m1, st1 = self.fifo_buffer.encode_sample(fifo_indices)
        s2, a2, a_m2, r2, c2, n_s2, d2, t2, i_s2, t_s2, m2, st2 = self.reservior_buffer.encode_sample(reservior_indices)
        ...
        return state, action, action_mean, reward, curiosity, next_state, done_mask, initial_state, time_step, mean, std
