...
class Custom_Res():
    ...
    def sample_individual(self, batch_size):
        ...
        for i in range(len(data)):
            state, action, action_mean, reward, curiosity, next_state, done_mask, t_array = data[i]
            transition_tuples.append(Transition_tuple(state, action, action_mean, reward, curiosity, next_state, done_mask, t_array))

        return transition_tuples

    def encode_n_sample_time(self, batch_size):
        indices = self.get_sample_indices_individual(batch_size)
