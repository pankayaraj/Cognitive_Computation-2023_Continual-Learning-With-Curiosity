...
class Debug():
    ...
    def print_all(self):
        print("ICM_LOSS = " + str(self.icm_next_state_loss.item()) + " , " + str(self.icm_action_loss.item())
              ...
              " , " + str(self.r_icm_r.item()))



class SAC_with_reward_based_Curiosity():
    ...
    def update(self, batch_size=None):
        ...
        icm_reward_loss = 0.5*torch.nn.functional.mse_loss(pred_reward, torch.FloatTensor(reward_batch).to(self.icm_nn_param.device))
        ...
        with torch.no_grad():
            ...
            r_icm_r = torch.nn.functional.mse_loss(p_r, reward_batch)

        self.debug.f_icm_r = f_icm_r
        self.debug.i_icm_r =  i_icm_r
        self.debug.r_icm_r = r_icm_r

        policy_loss = ((self.alpha*log_pi) - min_q_pi - r_icm_r).mean()
