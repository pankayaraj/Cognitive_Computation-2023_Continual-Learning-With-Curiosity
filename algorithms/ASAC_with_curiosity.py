...
class SAC_with_Curiosity():
    ...
    def update(self, batch_size=None):
        ...
        policy_loss = ((self.alpha*log_pi) ).mean()
    ...
    def step(self, state, random=False):
        ...
        s_c = torch.nn.functional.mse_loss(next_state, self.icm_nxt_state.get_next_state(state, action))
        a_c = torch.nn.functional.mse_loss(action, self.icm_action.get_action(state, next_state))
