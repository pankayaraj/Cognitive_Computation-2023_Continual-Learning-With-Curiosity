...
class ASAC():
    ...
    def __init__(self, env, q_nn_param, policy_nn_param, algo_nn_param, max_episodes =100, memory_capacity =10000,
                 ...
                 no_online_steps_per_update = 100,
                 ):
        ...
        self.no_online_steps_per_update = no_online_steps_per_update
        self.online_buffer = Replay_Memory(capacity=no_online_steps_per_update)
    ...
    def sample_mem(self, batch_size=None):
        if batch_size == None:
    ...
    def sample_online(self):
        online_batch = self.online_buffer.sample(batch_size=self.no_online_steps_per_update)

        return online_batch

    def update(self, batch, batch_size=None):
    ...
    def step(self, state, no_steps = None, random=False):
        if no_steps == None:
            no_steps = self.no_online_steps_per_update
        ...
        for _ in range(no_steps):
            ...
            self.online_buffer.push(state, action, action_mean, reward, next_state, mask)
