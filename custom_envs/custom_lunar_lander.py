...
for i in range(len(LANDER_POLY)):
    LANDER_POLY[i][0] = LANDER_POLY[i][0]//scale
    LANDER_POLY[i][1] = LANDER_POLY[i][1]//scale


LEG_AWAY = 20//scale
LEG_DOWN = 18//scale
LEG_W, LEG_H = 2/scale, 8//scale
LEG_SPRING_TORQUE = 40

SIDE_ENGINE_HEIGHT = 14.0//scale
SIDE_ENGINE_AWAY = 12.0//scale
...
class LunarLander(gym.Env, EzPickle):
    ...
    def __init__(self, main_engine_power = 13.0, side_engine_power = 0.6):
        ...
        self.power = self.MAIN_ENGINE_POWER
    ...
    def reset(self):
        ...
        height = np.ones(shape=(CHUNKS + 1,))*(H/2)
        chunk_x = [W/(CHUNKS-1)*i for i in range(CHUNKS)]

        indx = 3
        self.helipad_x1 = chunk_x[indx-1]
        self.helipad_x2 = chunk_x[indx+1]
        ...
        indx = 30
        height[indx - 2] = self.helipad_y
        height[indx - 1] = self.helipad_y
        height[indx + 0] = self.helipad_y
        height[indx + 1] = self.helipad_y
        height[indx + 2] = self.helipad_y
    ...
    def step(self, action):
        ...
        self.helipad_x_avg = (self.helipad_x1+self.helipad_x2)/2
        state = [
            (pos.x - VIEWPORT_W/SCALE/2) / (VIEWPORT_W/SCALE/2),
            #(pos.y - (self.helipad_x_avg/SCALE)) / (VIEWPORT_W/SCALE/4),
